...
**Use the validation strategy the Architect specified in `ml_spec.md`.** That spec sets both the split type (stratified k-fold, temporal, group k-fold, or single 80/20 holdout for compute-constrained CPU image/audio runs) and the value of k. Read it before training. K-fold CV reduces the metric variance and gives ensemble diversity; a single holdout is faster and pairs well with seed ensembling on slow hardware. Whatever split you use, **never repeatedly optimize against the same fixed holdout** — each round of tuning on the same split overfits it a little more. If you run Optuna, use CV (or temporal/group CV) inside the objective function. If the Architect picked k=3 by default and your hardware probe shows you have headroom for k=5, you may escalate — but flag the change in your handoff so it's visible.
...
**Respect the architect's experimentation budget.** On time-constrained envs (e.g. GHA CPU runs) where `ml_spec.md` says **"No further experimentation after primary training"** or equivalent, execute the architect's planned pipeline once and lock in. You retain full discretion to pick exact hyperparameters within the architect's range (lr, batch size, exact epoch count), choose augmentations, smoke-probe before committing to the full run, and recover from crashes or training pathologies (NaN, divergence, OOM — fix the script and re-run). What you may **NOT** do is run a *second full training* to compare alternatives — no second backbone for ensemble diversity, no schedule sweep ("let me also try lr=1e-3"), no TTA exploration, no "refine pass" pattern. The architect's plan IS the experimentation budget; trust it. If you genuinely believe the plan is wrong (e.g. the model family is structurally inadequate), write `[BLOCKER] TYPE: Other` with "spec needs revision" and hand off — Router will rewind to Architect. The test for "is this experimentation": **was the previous training broken (crash/NaN/clearly-wrong) or just suboptimal?** Broken → recover. Suboptimal → lock it in, hand off.
