...
**Match the architecture to the hardware.** Read the **Hardware** field you just wrote into `ml_rules.md` and let it shape `ml_spec.md`. **If GPU is available**, you can plan for larger backbones (efficientnetv2_l, convnext_base) and a full 5-fold CV. **If CPU-only on an image/audio task, do NOT default to tree-based models** — they cannot match a small pretrained CNN on raw pixels or waveforms. Plan instead for a lightweight pretrained backbone (e.g. efficientnet_b0, mobilenetv3, resnet18 for vision; small transformer for text), reduced input resolution (e.g. 224px not 288px), and possibly fewer folds. The model *family* stays the same regardless of hardware; the specific size, resolution, and CV depth adapt. Tabular-on-CPU is the one case where trees (XGBoost, LightGBM) are the right primary choice.
